resource "aws_instance" "websiteclone-host" {
  ...
  provisioner "local-exec" {
    when    = create
    command = "./create-ssh-config.sh '${var.hostname}' '${self.private_ip}' '${var.ssh_user}' './generated_key.pem' './ssh_config' 'false'"
  }
...
resource "null_resource" "run_ansbile_websiteclone" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook /home/ubuntu/Ansible/website-cloner/main.yml -i ${aws_instance.websiteclone-host[count.index].private_ip}, -e 'website_url=${var.website_url[count.index]}'"
    ]
  }
}
...
resource "null_resource" "run_ansbile_sslsetup" {
  ...
  provisioner "remote-exec" {
    inline = [
      "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook /home/ubuntu/Ansible/website-cloner/ssl-setup.yml -i ${aws_instance.websiteclone-host[count.index].private_ip}, -e 'DOMAIN=${var.domain_names[count.index]}'"
    ]
  }
}